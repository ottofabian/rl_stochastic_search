...
class Schwefel(BaseObjective):
    def __init__(self, dim, val_opt=4.2096874633 / 2, alpha=10):
        super(Schwefel, self).__init__(dim, val_opt=val_opt, alpha=alpha)
    ...
    def evaluate_full(self, x):
        ...
        return -1 / (100 * self.dim) * np.sum(z * np.sin(np.sqrt(np.abs(z))), axis=1) + 4.189828872724339 \
