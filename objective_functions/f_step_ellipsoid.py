...
class StepEllipsoid(BaseObjective):
    def __init__(self, dim, int_opt=(-5., 5.), alpha=10):
        super(StepEllipsoid, self).__init__(dim, int_opt=int_opt, alpha=alpha)
    ...
    def evaluate_full(self, x):
        ...
        out = 0.1 * np.maximum(np.abs(z_hat[0, :]) / 1e4,
                               np.sum(np.power(100, self.i / (self.dim - 1)) * z ** 2, axis=1)
