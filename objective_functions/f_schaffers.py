...
class Schaffers(BaseObjective):
    def __init__(self, dim, int_opt=(-5., 5.), alpha=10, beta=0.5):
        super(Schaffers, self).__init__(dim, int_opt=int_opt, alpha=alpha, beta=beta)
    ...
    def evaluate_full(self, x):
        ...
        out = (1 / (self.dim - 1) * np.sum(np.sqrt(s) + np.sqrt(s) * np.sin(50 * s ** 0.2) ** 2, axis=1)) ** 2 \
