...
class Gallagher21(BaseObjective):
    def __init__(self, dim):
        ...
        super(Gallagher21, self).__init__(dim, int_opt=int_opt, alpha=alpha)
        ...
        self.y = np.vstack([self.x_opt, np.random.uniform(int_loc_opt[0], int_loc_opt[1], size=(i, dim))])
    ...
    def evaluate_full(self, x):
        ...
        power = -1 / (2 * self.dim) * np.squeeze(z @ self.mat_fac[:, None, :, :] @ np.transpose(z, [0, 1, 3, 2]),
                                                 axis=(2, 3))
...
class Gallagher101(BaseObjective):
    def __init__(self, dim):
        ...
        alpha = np.hstack([alpha_1, np.random.choice(alpha_set, i, replace=False)])
        super(Gallagher101, self).__init__(dim, int_opt=int_opt, alpha=alpha)
        self.w = np.hstack([10, 1.1 + 8 * np.arange(i)/(i-1)])
        self.C = self.lambda_alpha / self.alpha[:, None, None]**0.25
        self.mat_fac = self.r.T @ self.C @ self.r
